...
function runDemo_gnocca_direct() {
  try {
    ...
    var tab1 = scriptProperties.getProperty('tab');
       ...
       //gnoccaforum
        var results = getReportDataForProfile_direct('ga:campaign',firstProfile,'ga:campaign=~gnoccaforum;ga:eventCategory==trafficoDirect');
    if (results!=undefined) {outputToSpreadsheet(results,'gnoccaforum');    
                            // outputToSpreadsheet_tot(results,'gnoccaforum');
                             outputToSpreadsheet_tot_row_ga_data(results,'risposta',tab1,56,17); // (...,...,tab,rowstart,colstart)
                            }
    else { printempty('gnoccaforum'); }
    
  
    //////gnoccaforum
  
    
       //gnoccatravelmassaggi
        var results = getReportDataForProfile_direct('ga:campaign',firstProfile,'ga:campaign=~gnoccatravelmassaggi;ga:eventCategory==trafficoDirect');
    if (results!=undefined) {outputToSpreadsheet(results,'gnoccatravelmassaggi');    
                             //outputToSpreadsheet_tot(results,'gnoccatravelmassaggi');
                             outputToSpreadsheet_tot_row_ga_data(results,'risposta',tab1,57,17); // (...,...,tab,rowstart,colstart)
                            }
    else { printempty('gnoccatravelmassaggi'); }
