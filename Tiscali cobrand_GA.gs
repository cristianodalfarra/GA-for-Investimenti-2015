...
function runDemo_tiscali() {
  try {
    ...
    var results = getReportDataForProfile_tiscali('ga:channelGrouping',firstProfile,"ga:campaign!=(not set)");
    if (results!=undefined)  {   outputToSpreadsheet(results,'Tiscali Cobrand');  //outputToSpreadsheet_tot(results,'Tiscali Cobrand'); 
                             outputToSpreadsheet_tot_row_ga_data(results,'',tab1,55,4);
                             }
    else { printempty('Tiscali Cobrand'); }
...
function getReportDataForProfile_tiscali(tipo,firstProfile,filters) {
