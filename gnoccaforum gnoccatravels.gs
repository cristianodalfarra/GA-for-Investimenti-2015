...
function runDemo_gnocca() {
  try {
    ...
    var tab1 = scriptProperties.getProperty('tab');
       ...
       //gnoccaforum
        var results = getReportDataForProfile_original('ga:campaign',firstProfile,'ga:campaign=~gnoccaforum');
    if (results!=undefined) {outputToSpreadsheet(results,'gnoccaforum');    
                            // outputToSpreadsheet_tot(results,'gnoccaforum');
                             outputToSpreadsheet_tot_row_ga_data(results,'risposta',tab1,56,4); // (...,...,tab,rowstart,colstart)
                            }
    ...
    //////gnoccaforum
       ...
       //gnoccatravelmassaggi
        var results = getReportDataForProfile_original('ga:campaign',firstProfile,'ga:campaign=~gnoccatravelmassaggi');
    if (results!=undefined) {outputToSpreadsheet(results,'gnoccatravelmassaggi');    
                             ...
                             outputToSpreadsheet_tot_row_ga_data(results,'risposta',tab1,57,4); // (...,...,tab,rowstart,colstart)
                            }
