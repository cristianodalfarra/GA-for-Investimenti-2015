...
function runDemo_FB_direct() {
  try {
    ...
    var tab1 = scriptProperties.getProperty('tab');
       ...
       //@FB_Servizi_
        var results = getReportDataForProfile_direct('ga:campaign',firstProfile,'ga:campaign=@FB_Servizi_;ga:eventCategory==trafficoDirect');
    if (results!=undefined) {outputToSpreadsheet(results,'FB_Servizi');    
                            
                             outputToSpreadsheet_tot_row_ga_data(results,'risposta',tab1,58,17); // (...,...,tab,rowstart,colstart)
                            }
    ...
    //////@FB_Servizi_
       ...
       //FacebookCartomanzia
        var results = getReportDataForProfile_direct('ga:campaign',firstProfile,'ga:campaign=~FacebookCartomanzia;ga:eventCategory==trafficoDirect');
    if (results!=undefined) {outputToSpreadsheet(results,'FacebookCartomanzia');    
                             
                             outputToSpreadsheet_tot_row_ga_data(results,'risposta',tab1,59,17); // (...,...,tab,rowstart,colstart)
                            }
...
//    var results = getReportDataForProfile_original('ga:campaign',firstProfile,'ga:campaign=~(facebookcamera|facebookmonolocale|facebookbilocale|facebooktrilocale|facebookaffittomain)');
...
//    else { printempty('Facebook Adv lavoro'); }
    ...
    //Facebook Adv Affitti
        var results = getReportDataForProfile_direct('ga:campaign',firstProfile,'ga:campaign=~(facebookcamera|facebookmonolocale|facebookbilocale|facebooktrilocale|facebookaffittomain);ga:eventCategory==trafficoDirect');
    if (results!=undefined) {outputToSpreadsheet(results,'Facebook Adv Affitti');    
                             
                             outputToSpreadsheet_tot_row_ga_data(results,'risposta',tab1,70,17); // (...,...,tab,rowstart,colstart)
                            }
    else { printempty('Facebook Adv Affitti'); }
    //////Facebook Adv Affitti
    
    //Facebook Adv Lavoro
        var results = getReportDataForProfile_direct('ga:campaign',firstProfile,'ga:campaign=~(facebookamministrazione|facebookanimazione),ga:campaign=~(facebooktrasporti|facebookinformatica|facebooksales|facebooklavoromain|facebookinsegnante);ga:eventCategory==trafficoDirect');
    if (results!=undefined) {outputToSpreadsheet(results,'Facebook Adv lavoro');    
                             
                             outputToSpreadsheet_tot_row_ga_data(results,'risposta',tab1,71,17); // (...,...,tab,rowstart,colstart)
                            }
    else { printempty('Facebook Adv Lavoro'); }
    /////Facebook Adv lavoro
