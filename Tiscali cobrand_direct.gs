...
function runDemo_tiscali_direct() {
  try {

    var firstProfile = getFirstProfile_tiscali();
    ...
    var tab1 = scriptProperties.getProperty('tab');
     ...
     var results = getReportDataForProfile_tiscali_direct('ga:channelGrouping',firstProfile,"ga:campaign!=(not set);ga:eventCategory==trafficoDirect");
    if (results!=undefined)  {   outputToSpreadsheet(results,'Tiscali Cobrand directCamp');  //outputToSpreadsheet_tot(results,'Tiscali Cobrand'); 
                             outputToSpreadsheet_tot_row_ga_data(results,'',tab1,55,17);
                             }
    else { printempty('Tiscali Cobrand directCamp'); }
...
function getFirstProfile_tiscali() {
...
function getReportDataForProfile_tiscali_direct(tipo,firstProfile,filters) {
